...
async function createCtr(formData: FormData) {
  ...
  const ClientName = formData.get("CustNameEn")?.toString();
  const ClientCode = formData.get("CustCode")?.toString();
  const CtrType = formData.get("CtrType")?.toString();
  const Currency = formData.get("Currency")?.toString();

  if (!CtrNumM || !DDate || !CustNameEn || !ClientCode || !CtrType || !Currency) {
