...
interface Column {
  title: string;
  buttons: ButtonProps[];
  order: number; // إضافة خاصية order لترتيب الأعمدة
}
...
const Page: React.FC = () => {
  ...
  return (
    <div className="mt-6 mb-16 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {columns.sort((a, b) => a.order - b.order).map((column, colIndex) => (
        <div key={colIndex} className={`flex flex-col items-center ${colIndex < 4 ? 'order-' + column.order : ''}`}>
          <h2 className="font-bold text-xl mb-2">{column.title}</h2>
          {column.buttons.map((button, index) => (
            <Button key={button.label} href={button.href} label={button.label} />
          ))}
        </div>
      ))}
    </div>
