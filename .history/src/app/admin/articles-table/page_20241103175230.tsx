...
interface AdminArticlesTableProps {
  searchParams: { pageNumber?: string }; // Made pageNumber optional
}

const AdminArticlesTable = async ({ searchParams }: AdminArticlesTableProps) => {
  // Ensure searchParams are resolved if needed
  const pageNumber = parseInt(searchParams?.pageNumber ?? "1");

  try {
    // Fetch articles and count with the appropriate page number
