...
const AdminArticlesTable = async ({ searchParams }: AdminArticlesTableProps) => {
  const pageNumber = parseInt(searchParams?.pageNumber ?? "1"); // هنا يتم التحقق وإعطاء قيمة افتراضية

  try {
    const articles: Article[] = await getArticles(pageNumber); // الحصول على المقالات بناءً على الصفحة المطلوبة
    const count: number = await prisma.article.count(); // حساب العدد الكلي للمقالات
    const pages = Math.ceil(count / ARTICLE_PER_PAGE); // حساب عدد الصفحات بناءً على عدد المقالات في كل صفحة
              ...
              <tr key={article.id} className="border-b border-t border-gray-300">
                ...
                <td>
                  <Link
                    ...
                    className="bg-green-600 text-white rounded-lg py-1 px-2 inline-block text-center mb-2 me-3 hover:bg-green-800 transition"
                  >
                    Edit
                  </Link>
