...
const SingleArticlePage = async ({ params }: singleArticlePageProps) => {
  ...
  let article: SingleArticle | null = null;

  try {
    article = await getSingleArticle(params.id);
  } catch (error) {
    console.error(error);
  }

  if (!article) {
    return <div className="text-red-500">Error fetching article. Please try again later.</div>;
  }
